...
while True:
	...
	if (time.time() - timerRM) > 60:
		timerRM = time.time()
		print("1min")
		mycursor.execute(f"select tp,ur,pressionelivellodelmare/100, wind*3.6, windAng, mmPioggia, rainrate, date, time, dewpoint from dataday where date = '{todayMYSQL}' ORDER BY TIMESTAMP(date,time) DESC LIMIT 1")
		...
		if data:
			...
			str3 = str2 + ";" + str(round(data[1], 1)) + ";" + str(round(data[3], 1)) + ";" + str(data[4]) + ";" + str(round(extr[2], 1))  + ";" + str(round(data[6], 1)) + ";" + str(round(data[5], 1)) + ";" + str(round(data[9], 1)) + ";IDEarduino;1.8.17;-99999;-99999;-99999;-99999; \n"
			meteonetwork.write(str3)
			print("row add to pmn175.txt")
			meteonetwork.close()

		'''mycursor.execute(f"select tp,pressionelivellodelmare/100, ur, windMedio*3.6, windAngMedio, rainrate, mmPioggia, dewpoint, date, time from dataday where date = '{todayMYSQL}' ORDER BY TIMESTAMP(date,time) DESC LIMIT 1")
		MN = mycursor.fetchone()
		mycursor.execute(f"select max(windMax)*3.6 from dataday where date = '{todayMYSQL}'")
		MNmaxwind = mycursor.fetchone()
		mydb.commit()'''
		#controllo inserimento data
		mycursor.execute(f"SELECT UNIX_TIMESTAMP(TIMESTAMP(date,time)) FROM dataday ORDER BY TIMESTAMP(date,time) DESC LIMIT 1")
		VE = mycursor.fetchone()
		mydb.commit()

		#print(VE)
		#print(time.time() - VE[0])

		if (time.time() - VE[0] ) > 600 and dooo:
			dooo = False
			#email stuff
