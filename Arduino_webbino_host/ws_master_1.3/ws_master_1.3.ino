...
radioTx.ersd  = SD.begin(SD_SS, SPI_HALF_SPEED); 
  ...
  /*    String dataString = String(year()) + "/" + String(month()) + "/" + String(day()) + " " + String(hour()) + ":" + String(minute()) + " " + String(tp, 1) + " " + String(tpdht, 1) + " " +
      ...
      File logFile = SD.open("dataDay.csv", FILE_WRITE);
      if (logFile){
        float dataday[] = {tp, tpdht, ur, dewPoint, heatindexc, wind.valmedio*3.6, wind.max*3.6, gradi[radioTx.winddirMedio], windchill, mmPioggia, rainrate, RRmax, tp2};
        logFile.print(day());
        logFile.print("/");
        logFile.print(month());
        logFile.print("/");
        ...
        logFile.print(" ");
        for (int i=0; i<sizeof(dataday)-1; i++) {
          logFile.print(dataday[i],1);
          logFile.print(" ");
        }
        logFile.println(dataday[sizeof(dataday)-1], 1);
      }
      logFile.close();  
...
/*
 ...
 //rtc.adjust(DateTime(F(__DATE__),F(__TIME__)));
  rtc.adjust(DateTime(2022,9,26,19,13,0)); //(impostare -1 ora )
*/
}
